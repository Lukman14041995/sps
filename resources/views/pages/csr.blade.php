        ...
        <div class="relative h-[300px] sm:h-[350px] md:h-[400px] lg:h-[450px] xl:h-[500px] overflow-hidden">
            ...
            <div class="absolute inset-0">
                <img src="https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80"
                    class="w-full h-full object-cover" alt="SPS Corporate CSR" style="object-position: center 30%;"
                    loading="lazy">
                <!-- Gradient Overlay -->
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div>
                        <div class="mb-8 sm:mb-10 lg:mb-12">
                            ...
                            <div class="w-20 h-1 bg-gradient-to-r from-blue-500 to-blue-700 rounded-full mb-6 sm:mb-8">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 mt-8 sm:mt-10">
                            ...
                            <div class="text-center">
                                <div class="text-2xl sm:text-3xl font-bold text-blue-700 mb-1">
                                    {{ number_format($totalBeneficiaries) }}+</div>
                    ...
                    <div class="relative">
                        <div
                            class="relative rounded-xl sm:rounded-2xl lg:rounded-3xl overflow-hidden shadow-lg sm:shadow-xl group">
                            @if ($latestCsr = \App\Models\Csr::published()->latest()->first())
                                <img src="{{ Storage::disk('s3')->url($latestCsr->featured_image) }}"
                                    ...
                                    alt="{{ $latestCsr->title }}" loading="lazy">
                            @else
                                <img src="https://images.unsplash.com/photo-1509099836639-18ba1795216d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80"
                                    class="w-full h-64 sm:h-72 md:h-80 lg:h-[400px] object-cover group-hover:scale-105 transition-transform duration-700"
                                    alt="SPS Corporate CSR Activity" loading="lazy">
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent"></div>

                            <!-- Impact Card -->
                            <div
                                class="absolute -bottom-4 sm:-bottom-6 -left-4 sm:-left-6 bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg sm:shadow-xl w-48 sm:w-56 lg:w-64">
                                <div class="text-center">
                                    <div class="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-600 mb-1 sm:mb-2">
                                        {{ $yearsActive }}+</div>
                                    <div class="text-xs sm:text-sm text-gray-600 font-semibold">Tahun Berkomitmen</div>
                                    <div
                                        class="w-12 sm:w-16 h-1 bg-gradient-to-r from-green-500 to-green-700 mx-auto mt-2 sm:mt-3 rounded-full">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8" id="csrGrid">
                    @forelse($csrPrograms as $csr)
                        <article
                            class="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 csr-card"
                            data-category="{{ $csr->category }}" data-year="{{ $csr->year }}">
                            <!-- Image -->
                            <div class="relative overflow-hidden h-48">
                                <img src="{{ asset('storage/' . $csr->featured_image) }}"
                                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                                    alt="{{ $csr->title }}" loading="lazy">
                                ...
                                <div class="absolute top-4 left-4">
                                    ...
                                    @endphp
                                    <span
                                        class="px-3 py-1 {{ $categoryColors[$csr->category] ?? 'bg-blue-600/90' }} text-white text-xs font-semibold rounded-full">
                                ...
                                <div class="absolute bottom-4 right-4">
                                    <div class="bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-sm">
                                        <div class="text-xs font-bold text-gray-900">
                                            {{ \Carbon\Carbon::parse($csr->created_at)->format('d') }}</div>
                                        <div class="text-xs text-gray-600">
                                            {{ \Carbon\Carbon::parse($csr->created_at)->format('M') }}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                <!-- Title -->
                                <h3
                                    class="text-lg sm:text-xl font-bold text-gray-900 mb-3 group-hover:text-blue-700 transition-colors">
                                    {{ $csr->title }}
                                </h3>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-6xl mx-auto">
                ...
                <div class="grid md:grid-cols-3 gap-6 lg:gap-8">
                    @foreach ($categories as $key => $category)
                        <div
                            class="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
                            <div class="p-6 sm:p-8">
                                <!-- Icon -->
                                <div
                                    class="w-16 h-16 {{ $category['bg_color'] }} rounded-2xl flex items-center justify-center mb-6 mx-auto">
                                    {!! $category['icon'] !!}
                                </div>

                                <!-- Title -->
                                <h3
                                    class="text-xl sm:text-2xl font-bold text-gray-900 mb-4 text-center group-hover:{{ $category['text_color'] }} transition-colors">
                                    {{ $category['name'] }}
                                </h3>
                                ...
                                <div class="text-center">
                                    ...
                                    @endphp
                                    <div class="text-2xl font-bold {{ $category['text_color'] }}">{{ $count }}
                                    </div>
