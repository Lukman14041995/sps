...
@section('title', 'Master Bisnis Kategori')
    ...
    <div class="w-full bg-gray-50 px-2 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm mb-4 px-3 sm:px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-lg sm:text-xl font-bold text-gray-900">Master Bisnis Kategori</h1>
                    <p class="text-gray-500 text-xs sm:text-sm">Manajemen kategori bisnis</p>
                </div>
                <a href="{{ route('admin.bisnis-kategori.create') }}"
                    ...
                    @forelse($data as $i => $row)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-center whitespace-nowrap">
                                ...
                                <a href="{{ route('admin.bisnis-kategori.edit', $row->id) }}"
                                ...
                                <form id="delete-form-{{ $row->id }}"
                                    action="{{ route('admin.bisnis-kategori.destroy', $row->id) }}" method="POST"
                                    class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
