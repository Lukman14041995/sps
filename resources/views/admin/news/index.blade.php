...
<div class="bg-white rounded-xl shadow-lg border border-gray-200">
    ...
    <div class="p-6 border-b border-gray-200 bg-gray-50">
        <form method="GET" action="#" class="space-y-4 md:space-y-0 md:grid md:grid-cols-5 md:gap-4">
            <!-- Search -->
    ...
    <div class="p-4 border-b border-gray-200 bg-yellow-50 hidden" id="bulkActionsContainer">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <select id="bulkAction" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none">
        ...
        <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <!-- Status -->
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 text-xs rounded-full {{ $item->status == 'published' ? 'bg-green-100 text-green-800' : ($item->status == 'draft' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800') }}">
                            {{ ucfirst($item->status) }}
                        </span>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <span class="font-medium">{{ number_format($item->views) }}</span>
                            @if($item->likes > 0)
                            <span class="ml-4">
...
<script>
    ...
    function updateStatus(newsId, status) {
        ...
        fetch(`/admin/news/${newsId}/update-status`, {
                method: 'POST',
                headers: {
